<form method="post" action="index.php?controller=utilisateur&action=<?php echo $target_action;?>">
    <fieldset>
        ...
        <p>
            ...
            <input type="password" placeholder="Ex : ******" name="mdp" id="mdp_id" value="<?php echo htmlspecialchars($v->get('mdp'));?>" required/>
        </p>

        <p>
            <label for="mdpConfirm_id">Vérifier le Mot de Passe</label> :
            <input type="password" placeholder="Ex : ******" name="mdpConfirm" id="mdpConfirm_id" required/>
        </p>
